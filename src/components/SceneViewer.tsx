    ...
    <Box sx={{ p: 4 }}>
        ...
        <Paper sx={{ p: 3, mb: 4 }}>
          ...
          {selectedSolution.autoGenerated && (
            <Alert severity="info" sx={{ mb: 2 }}>
              此方案由 {selectedSolution.generator || 'unknown'} 自动生成
            </Alert>
          )}
                    ...
                    {kp.tradeOffs && (
                      <Box sx={{ mt: 1 }}>
                        <Typography variant="caption" color="text.secondary">
                          权衡: {Object.entries(kp.tradeOffs).map(([k, v]) => `${k}: ${v}`).join(', ')}
                        </Typography>
                      </Box>
                    )}
                  </Box>
      ...
      {scene.solutions.length > 1 && (
        <Paper sx={{ p: 3 }}>
          ...
          <Typography variant="body2" color="text.secondary">
            此场景包含 {scene.solutions.length} 个解决方案，可通过上方标签切换查看。
          </Typography>
        </Paper>
      )}
    </Box>
